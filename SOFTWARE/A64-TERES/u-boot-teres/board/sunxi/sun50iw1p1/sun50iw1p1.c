...
gpio_scl=0;
gpio_sda=0;
ret= fdt_get_one_gpio(FDT_LCD0_PATH, "lcd_soft_gpio_scl",(void *)&pio_data);
if(ret >= 0)
            {
                printf("[lcd0] SCL GPIO requested !!\n");
                gpio_scl = gpio_request((void *)&pio_data, 1);
                gpio_write_one_pin_value(gpio_scl, 0,0);
  }
ret= fdt_get_one_gpio(FDT_LCD0_PATH, "lcd_soft_gpio_sda",(void *)&pio_data);
if(ret >= 0)
            {
                printf("[lcd0]SDA GPIO requested !!\n");
                gpio_sda = gpio_request((void *)&pio_data, 1);
                gpio_write_one_pin_value(gpio_sda, 0,0);
  }

soft_i2c_gpio_scl = gpio_scl;
soft_i2c_gpio_sda = gpio_sda;
        return ret;
}
*/
