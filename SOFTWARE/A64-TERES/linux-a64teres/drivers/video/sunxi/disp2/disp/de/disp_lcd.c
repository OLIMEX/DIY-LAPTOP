...
struct disp_device* disp_get_lcd(u32 disp)
{
	...
	if (disp >= num_screens || !bsp_disp_feat_is_supported_output_types(disp, DISP_OUTPUT_TYPE_LCD)) {
		printk("disp %d not support lcd output\n", disp);
		return NULL;
	}
...
static s32 lcd_parse_panel_para(u32 disp, disp_panel_para * info)
{
    ...
    ret = disp_sys_script_get_item(primary_key, "lcd_model_name", (int*)info->lcd_model_name, 2);
    		...
    		printk("[DEV]*** \n");
                ...
                printk("[DEV]*** lcd_y:      %d\n", info->lcd_y);
                printk("[DEV]*** lcd_ht:     %d\n", info->lcd_ht);
                printk("[DEV]*** lcd_hbp:    %d\n", info->lcd_hbp);
                printk("[DEV]*** lcd_vt:     %d\n", info->lcd_vt);
                printk("[DEV]*** lcd_vbp:    %d\n", info->lcd_vbp);
                printk("[DEV]*** lcd_hspw:   %d\n", info->lcd_hspw);
                printk("[DEV]*** lcd_vspw:   %d\n", info->lcd_vspw);
                printk("[DEV]*** lcd_if      %d\n", info->lcd_if);
                printk("[DEV]*** lcd_hv_if   %d\n", info->lcd_hv_if);
		printk("[DEV]*************************************\n");
...
/*	if ((NULL == lcd) || (NULL == lcdp)) {
	...
	printk("[DEV] lcd_pin_cfg \n");
	printk("[DEV] lcd %d pin config, state %s, %d\n", lcd->disp, (bon)? "on":"off", bon);
...
static s32 disp_lcd_enable(struct disp_device* lcd)
{
	...
	printk("[DEV][disp_lcd_enable]lcd %d\n", lcd->disp);
	mgr = lcd->manager;
	if ((NULL == mgr)) {
		DE_WRN("mgr is NULL!\n");
		return DIS_FAIL;
	...
	if (lcdp->lcd_panel_fun.cfg_panel_info)
		...
		printk("lcd_panel_fun[%d].cfg_panel_info is NULL\n", lcd->disp);
...
s32 disp_init_lcd(disp_bsp_init_para * para)
{
	...
	if (NULL == lcds) {
		printk("[DEV]malloc memory(%d bytes) fail!\n",
...
//	 char dev_name[25];	
//	sprintf(dev_name, "lcd",0 );
////        disp_sys_pin_set_state("lcd0","active");

//        disp_al_lcd_io_cfg(lcd->disp, 1, &lcdp->panel_info);

	printk("[DEV] disp_init_lcd -> for loop \n");
...
#endif
		printk("[DEV]lcd %d, irq_no=%d, irq_no_dsi=%d\n", disp, lcdp->irq_no, lcdp->irq_no_dsi);

		lcd->set_manager = disp_device_set_manager;
		printk("[DEV] disp_init_lcd -> disp_device_set_manager\n");
		lcd->unset_manager = disp_device_unset_manager;
		 printk("[DEV] disp_init_lcd -> disp_device_unset_manager\n");
		lcd->get_resolution = disp_device_get_resolution;
		 printk("[DEV] disp_init_lcd -> disp_device_get_resolution\n");
		lcd->get_timings = disp_device_get_timings;
		 printk("[DEV] disp_init_lcd -> disp_device_get_timings\n");
		lcd->enable = disp_lcd_enable;
		 printk("[DEV] disp_init_lcd -> disp_lcd_enable\n");
		lcd->sw_enable = disp_lcd_sw_enable;
		printk("[DEV] disp_init_lcd -> disp_lcd_sw_enable\n");
		lcd->fake_enable = disp_lcd_fake_enable;
		printk("[DEV] disp_init_lcd -> disp_lcd_fake_enable\n");
		lcd->disable = disp_lcd_disable;
		printk("[DEV] disp_init_lcd -> disp_lcd_disable\n");
		lcd->is_enabled = disp_lcd_is_enabled;
		printk("[DEV] disp_init_lcd -> disp_lcd_is_enabled\n");
		lcd->set_bright = disp_lcd_set_bright;
		printk("[DEV] disp_init_lcd -> disp_lcd_set_bright\n");
		lcd->get_bright = disp_lcd_get_bright;
		printk("[DEV] disp_init_lcd -> disp_lcd_get_bright\n");
		lcd->set_bright_dimming = disp_lcd_set_bright_dimming;
		printk("[DEV] disp_init_lcd -> disp_lcd_set_bright_dimming\n");
		lcd->get_panel_info = disp_lcd_get_panel_info;
                printk("[DEV] disp_init_lcd -> disp_lcd_get_panel_info\n");
		lcd->set_panel_func = disp_lcd_set_panel_funs;
		printk("[DEV] disp_init_lcd -> disp_lcd_set_panel_funs\n");
		lcd->set_open_func = disp_lcd_set_open_func;
		 printk("[DEV] disp_init_lcd -> disp_lcd_set_open_funs\n");
		lcd->set_close_func = disp_lcd_set_close_func;
		printk("[DEV] disp_init_lcd -> disp_lcd_set_close_funs\n");
		lcd->backlight_enable = disp_lcd_backlight_enable;
		printk("[DEV] disp_init_lcd -> disp_lcd_backlight_enable\n");
		lcd->backlight_disable = disp_lcd_backlight_disable;
		printk("[DEV] disp_init_lcd -> disp_lcd_backlight_disable\n");
		lcd->pwm_enable = disp_lcd_pwm_enable;
		printk("[DEV] disp_init_lcd -> disp_lcd_pwm_enable\n");
		lcd->pwm_disable = disp_lcd_pwm_disable;
		printk("[DEV] disp_init_lcd -> disp_lcd_pwm_disable\n");
		lcd->power_enable = disp_lcd_power_enable;
		printk("[DEV] disp_init_lcd -> disp_lcd_power_enable\n");
		lcd->power_disable = disp_lcd_power_disable;
		printk("[DEV] disp_init_lcd -> disp_lcd_power_disable\n");
		lcd->pin_cfg = disp_lcd_pin_cfg;
		printk("[DEV] disp_init_lcd -> disp_lcd_pin_cfg\n");
		lcd->tcon_enable = disp_lcd_tcon_enable;
		printk("[DEV] disp_init_lcd -> disp_lcd_tcon_enable\n");
		lcd->tcon_disable = disp_lcd_tcon_disable;
		printk("[DEV] disp_init_lcd -> disp_lcd_tcon_disable\n");
		lcd->gpio_set_value = disp_lcd_gpio_set_value;
		printk("[DEV] disp_init_lcd -> disp_lcd_gpio_set\n");
		lcd->gpio_set_direction = disp_lcd_gpio_set_direction;
		printk("[DEV] disp_init_lcd -> disp_lcd_gpio_set_directions\n");
		lcd->get_dimensions = disp_lcd_get_dimensions;
		printk("[DEV] disp_init_lcd -> disp_lcd_get_dimensions\n");
		lcd->get_status = disp_lcd_get_status;
		printk("[DEV] disp_init_lcd -> disp_lcd_get_status\n");
		lcd->init = disp_lcd_init;
		printk("[DEV] disp_init_lcd -> disp_lcd_init\n");
		lcd->exit = disp_lcd_exit;
		printk("[DEV] disp_init_lcd -> disp_lcd_exit\n");
		if (bsp_disp_feat_is_supported_output_types(disp, DISP_OUTPUT_TYPE_LCD)) {
			lcd->init(lcd);
			disp_device_register(lcd);
			printk("[DEV] disp_init_lcd -> disp_device_register\n");
		}
	}
