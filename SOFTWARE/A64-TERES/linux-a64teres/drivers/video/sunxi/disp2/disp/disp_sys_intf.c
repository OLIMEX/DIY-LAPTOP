...
{
	printk("%s, irqNo=%d, Handler=0x%p, pArg=0x%p\n", __func__, IrqNo, Handler, pArg);
...
int disp_sys_gpio_request(disp_gpio_set_t *gpio_list, u32 group_count_max)
{
	...
	if (0 != ret) {
		...
		printk("[DEV]%s, gpio_name=%s, gpio=%d, <%d,%d,%d,%d>ret=%d\n", __func__, gpio_list->gpio_name, gpio_list->gpio,\
...
int disp_sys_pin_set_state(char *dev_name, char *name)
{
	...
	int ret = -1;
	printk("[DEV] Init PinState\n");
	len = sprintf(compat, "allwinner,sunxi-%s", dev_name);
	if (len > 32)
		printk("[DEV][PIN] size of mian_name is out of range\n");

	node = of_find_compatible_node(NULL, NULL, compat);
	if (!node) {
		printk("[DEV][PIN] of_find_compatible_node %s fail\n", compat);
		goto exit;
	}

	pdev = of_find_device_by_node(node);
	if (!node) {
		printk("[DEV][PIN] of_find_device_by_node for %s fail\n", compat);
		goto exit;
	}

	pctl = pinctrl_get(&pdev->dev);
	if (IS_ERR(pctl)) {
		printk("[DEV][PIN] pinctrl_get for %s fail\n", compat);
		ret = PTR_ERR(pctl);
		goto exit;
	}
	...
	if (IS_ERR(state)) {
		printk("[DEV][PIN] pinctrl_lookup_state for %s fail\n", compat);
		ret = PTR_ERR(state);
		goto exit;
	}
	...
	if (ret < 0) {
		printk("[DEV][PIN] pinctrl_select_state(%s) for %s fail\n", name, compat);
		goto exit;
	} else 
		{
	 printk("[DEV][PIN] pinctrl_select_state(%s) for %s OK\n", name, compat);
